...
function defaultTitleFormatter(options) {
  const { timestamp, duration, flags, logFirstPart } = options;

  return (action = {}, time, took) => {
    const parts = [ logFirstPart || 'widget-action' ];

    if (flags) {
      const actionFlags = get(action, 'metadata.flags', []);

      if (Array.isArray(actionFlags) && actionFlags.length > 0) {
        parts.push(`%c${actionFlags.join(', ')}`);
      }
    }
...
function printBuffer(buffer, options) {
  ...
  buffer.forEach((logEntry, key) => {
    ...
    const formattedTime = startedTime && formatTime(startedTime);
    const titleCSS = colors.title ? `color: ${colors.title(formattedAction)};` : '';
    const headerCSS = [ 'color: gray; font-weight: lighter;' ];

    if (options.flags && logEntry.flags) headerCSS.push('color: coral;');
